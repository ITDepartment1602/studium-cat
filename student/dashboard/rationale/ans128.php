...
if (!empty($eid) && !empty($kilanlan) && !empty($qq)) {
  ...
  if ($result->num_rows > 0) {
    ...
    $options = json_decode($fetch['options'], true); // Decode options
    $correctAnswer = json_decode($fetch['correctans'], true); // Decode correct answers

    // Ensure correctAnswer is an array
    if (!is_array($correctAnswer)) {
      $correctAnswer = []; // Default to an empty array if decoding fails
    }
  } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $ans = $_POST['ans'] ?? []; // Get the answer from the form
  ...
  if ($row = $result->fetch_assoc()) {
    $correctAns = json_decode($row['correctans'], true); // Decode the correct answers

    // Initialize counts
    if (!is_array($correctAns)) {
      $correctAns = []; // Default to an empty array if decoding fails
    }
    ...
    $isCorrect = 1; // Assume correct unless any answer is wrong
    foreach ($ans as $answer) {
      if (!in_array($answer, $correctAns)) {
        $isCorrect = 0; // Mark as incorrect if any selected answer is not correct
        $wrongCount++;
      } else {
        $correctCount++; // Increment correct count for valid selections
      }
    }
    ...
    $encodedAns = is_array($ans) ? json_encode($ans) : json_encode([]);

    if ($count == 0) {

      $jsonCorrectAns = json_encode($correctAns);
      ...
      $questionNumber = 128; // KASUKASUAN
      $stmt->bind_param("iissssisssi", $qq, $isCorrect, $fetch['topics1'], $fetch['system'], $fetch['cnc'], $timeTaken, $user_id, $examTaken, $encodedAns, $jsonCorrectAns, $questionNumber);
      $stmt->execute();
    } else {
      ...
      $stmt->bind_param("iissis", $isCorrect, $timeTaken, $encodedAns, $qq, $examTaken, $user_id);
      $stmt->execute();
    }
  }
}
      ...
      <div class="main_containerab">
        <div class="contentab">
          ...
          <div class="content1ab">
            ...
            $select = mysqli_query($con, "SELECT * FROM `question` WHERE topics1 = '$eid' AND topic = '$kilanlan' AND id = '$qq' AND type = 'SATA'") or die('query failed');
            ...
            <form>
              <br>
              <b style="font-size: 20px; ">Question 128</b><br><br>
              <?= $fetch['question'] ?? 'Question not found' ?><strong> Select all that
                apply.</strong><br><br>
              <!-- QUESTION Choices UPDATED COPY STARTS HERE -->
              <?php
              $correctAnswer = json_decode($fetch['correctans'], true) ?? []; // Decode and ensure it's an array
              $selectedAnswer = isset($_POST['ans']) ? array_map('intval', $_POST['ans']) : []; // Convert to integers
        ...
        background-color: <?= in_array($index, $correctAnswer) ? '#00B11B' : '#F2F2F2' ?>;">
                  ...
                  <!-- Read-only Checkbox -->
                  <input type="checkbox" name="ans[]" value="<?= $index ?>" <?= in_array($index, $selectedAnswer) ? 'checked' : 'disabled' ?> style="margin-right: 10px; cursor: pointer;" disabled>
          ...
          <div class="footerab">
            ...
            ?>
            <form method="POST"
              action="../question/question129.php?topics1=<?= $_GET['eid'] ?>&topics2=<?= $_GET['topics2'] ?>&kilanlan=<?= $_GET['kilanlan'] ?>&id=<?= $_GET['id'] ?>&kilanlanhistory=<?= $_GET['kilanlanhistory'] ?>&cc=<?= $correctCount ?>&wc=<?= $wrongCount ?>&qnums=<?= $qnumsString ?>">
